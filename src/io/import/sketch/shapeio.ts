...
function importPoints(data: IJSON): CurvePoint[] {
    return (data['points'] || []).map((d: IJSON, i: number) => {
        ...
        const p = new CurvePoint(new CrdtIndex([i], 0), uuid(), point.x, point.y, curveMode);
        if (hasCurveFrom) {
            p.hasFrom = true;
            p.fromX = curveFrom.x;
            p.fromY = curveFrom.y;
...
export function importArtboard(ctx: LoadContext, data: IJSON, f: ImportFun, i: number): Artboard {
    ...
    if (hasBackgroundColor && backgroundColor) {
        const fill = new Fill(new CrdtIndex([0], 0), uuid(), true, FillType.SolidColor, backgroundColor);
        style.fills.length = 0;
        style.fills.push(fill);
    } else {
        const fill = new Fill(new CrdtIndex([0], 0), uuid(), true, FillType.SolidColor, new Color(1, 255, 255, 255));
        style.fills.length = 0;
        style.fills.push(fill);
    }
    const childs = (data['layers'] || []).map((d: IJSON, i: number) => f(ctx, d, i));
    const shape = new Artboard(new CrdtIndex([i], 0), id, name, ShapeType.Artboard, frame, style, new BasicArray<Shape>(...childs));
...
export function importGroupShape(ctx: LoadContext, data: IJSON, f: ImportFun, i: number): GroupShape {
    ...
    const shape = new GroupShape(new CrdtIndex([i], 0), id, name, ShapeType.Group, frame, style, new BasicArray<Shape>(...childs));
    importShapePropertys(shape, data);
    importBoolOp(shape, data);
    shape.exportOptions = exportOptions;
    return shape;
...
export function importImage(ctx: LoadContext, data: IJSON, f: ImportFun, i: number): ImageShape {
    ...
    const shape = new ImageShape(new CrdtIndex([i], 0), id, name, ShapeType.Image, frame, style, curvePoint, true, imageRef);
...
export function importPage(ctx: LoadContext, data: IJSON, f: ImportFun): Page {
    ...
    const childs: Shape[] = (data['layers'] || []).map((d: IJSON, i: number) => f(ctx, d, i));
    const shape = new Page(new CrdtIndex([], 0), id, name, ShapeType.Page, frame, style, new BasicArray<Shape>(...childs));
...
export function importPathShape(ctx: LoadContext, data: IJSON, f: ImportFun, i: number): PathShape {
    ...
    const shape = new PathShape(new CrdtIndex([i], 0), id, name, ShapeType.Path, frame, style, new BasicArray<CurvePoint>(...points), isClosed);
    importShapePropertys(shape, data);
    importBoolOp(shape, data);
    shape.exportOptions = exportOptions;
    return shape;
...
export function importRectShape(ctx: LoadContext, data: IJSON, f: ImportFun, i: number): RectShape {
    ...
    const shape = new RectShape(new CrdtIndex([i], 0), id, name, ShapeType.Rectangle, frame, style, new BasicArray<CurvePoint>(...points), true);
    importShapePropertys(shape, data);
    importBoolOp(shape, data);
    shape.exportOptions = exportOptions;
    return shape;
...
export function importTextShape(ctx: LoadContext, data: IJSON, f: ImportFun, i: number): TextShape {
    ...
    // const isClosed = data['isClosed'];
    const shape = new TextShape(new CrdtIndex([i], 0), id, name, ShapeType.Text, frame, style, text);
    importShapePropertys(shape, data);
    importBoolOp(shape, data);
    shape.exportOptions = exportOptions;
    return shape;
...
export function importSymbol(ctx: LoadContext, data: IJSON, f: ImportFun, i: number): SymbolShape {
    ...
    const childs: Shape[] = (data['layers'] || []).map((d: IJSON, i: number) => f(ctx, d, i));
    const shape = new SymbolShape(new CrdtIndex([i], 0), id, name, ShapeType.Symbol, frame, style, new BasicArray<Shape>(...childs), new CrdtId("", 0), new BasicMap());
...
export function importSymbolRef(ctx: LoadContext, data: IJSON, f: ImportFun, i: number): SymbolRefShape {
    ...
    const shape = new SymbolRefShape(new CrdtIndex([i], 0), id, name, ShapeType.SymbolRef, frame, style, data['symbolID'], new BasicMap());
