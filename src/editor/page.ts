...
export class PageEditor {
    ...
    group(shapes: ShapeView[], groupname: string): false | GroupShape { // shapes中元素index越小层级越高，即在图形列表的位置最高
        ...
        try {
            ...
            gshape = group(this.__document, this.page, shapes.map(s => adapt2Shape(s)), gshape, savep, saveidx, op);
            this.__repo.commit();
            return gshape;
        } catch (e) {
            console.log(e)
    ...
    ungroup(shapes: GroupShapeView[]): false | Shape[] {
        ...
        try {
            for (let i = 0; i < shapes.length; i++) {
                ...
                const childs = ungroup(this.__document, this.page, adapt2Shape(shape) as GroupShape, op);
    ...
    createArtboard(shapes: ShapeView[], artboardname: string): Artboard {
        try {
            ...
            const saveidx = savep.indexOfChild(adapt2Shape(shapes[0]));
            artboard = group(this.__document, this.page, shapes.map(s => adapt2Shape(s)), artboard, savep, saveidx, op) as Artboard;
            this.__repo.commit();
            return artboard;
        } catch (e) {
            this.__repo.rollback();
    ...
    dissolution_artboard(shapes: ArtboardView[]): Shape[] {
        try {
            ...
            for (let i = 0; i < shapes.length; i++) {
                ...
                if (shape.isVirtualShape) continue;
                if (!shape.parent) continue;
                const childs = ungroup(this.__document, this.page, adapt2Shape(shape) as Artboard, op);
    ...
    create_autolayout_artboard(shapes: ShapeView[], artboardname: string): Artboard | void {
        ...
        try {
            ...
            artboard = group(this.__document, this.page, childs.map(s => adapt2Shape(s)), artboard, savep, saveidx, op) as Artboard;
            this.__repo.commit();
            return artboard;
        } catch (e) {
            this.__repo.rollback();
    ...
    modifyShapesContextSettingOpacity(shapes: ShapeView[], value: number) {
        ...
        try {
            const op = this.__repo.start("modifyShapesContextSettingOpacity");
            const page = this.page;
            for (const view of shapes) {
                const shape = shape4contextSettings(op, view, this.view);
    ...
    modifyShapesContextSettingBlendMode(shapes: Shape[], blendMode: types.BlendMode) {
        try {
            const op = this.__repo.start("modifyShapesContextSettingBlendMode");
            for (let i = 0, l = shapes.length; i < l; i++) {
                const item = shapes[i];
                op.shapeModifyContextSettingsBlendMode(this.page, item, blendMode);
            }
            this.__repo.commit();
            return true;
        } catch (e) {
    ...
    boolgroup(shapes: Shape[], groupname: string, op: BoolOp) {
        ...
        try {
            ...
            gshape = group(this.__document, this.page, shapes, gshape, savep, saveidx, operator);
            shapes.forEach((shape) => operator.shapeModifyBoolOp(this.page, shape, op))
            this.__repo.commit();
            return gshape;
        } catch (e) {
            this.__repo.rollback();
    ...
    boolgroup2(savep: GroupShape, groupname: string, op: BoolOp): false | BoolShape {
        try {
            ...
            gshape = group(this.__document, this.page, shapes, gshape, pp, saveidx, operator);
            // 上面group会删除空的编组对象，需要再判断下对象是否还在
            saveidx = pp.indexOfChild(savep);
            if (saveidx >= 0) operator.shapeDelete(this.__document, this.page, pp, saveidx);
            shapes.forEach((shape) => operator.shapeModifyBoolOp(this.page, shape, op))
    ...
    makeSymbol(document: Document, views: ShapeView[], name?: string) {
        try {
            ...
            if (replace) {
                ...
                sym = op.shapeInsert(document, page, shape0.parent as GroupShape, symbolShape, index + 1) as SymbolShape;
                const children = shape0.childs;
                for (let i = 0, len = children.length; i < len; ++i) {
                    op.shapeMove(page, shape0, 0, symbolShape, i);
                }
                op.shapeDelete(document, page, shape0.parent as GroupShape, index);
            } else {
                const index = (shape0.parent as GroupShape).indexOfChild(shape0);
                symbolShape.frameMaskDisabled = true;
                sym = group(document, page, shapes, symbolShape, shape0.parent as GroupShape, index, op);
                const groupFrame = FrameCpt.frames2RootBound(views);
                op.shapeModifyWH(page, sym, groupFrame.width, groupFrame.height);
                for (const shape of shapes) {
                    ...
                    op.shapeModifyResizingConstraint(page, shape, new_rc);
                }
            }
            ...
            if (innerSymbols.length) { // replace
                ...
                for (const symbol of innerSymbols) {
                    ...
                    op.shapeInsert(document, page, parent, ref, parent.indexOfChild(symbol));
                    const om = symbol.matrix2Root();
                    om.trans(offset, 0);
                    om.multiAtLeft(matrixToPage);

                    op.shapeMove(page, parent, parent.indexOfChild(symbol), page, page.childs.length);
                    op.shapeModifyTransform(page, symbol, ((om)));
                    ...
                    if ([ShapeType.Artboard, ShapeType.Symbol, ShapeType.SymbolRef].includes(parent.type)) {
                        ...
                        for (let j = 0; j < sortedArr.length; j++) {
                            ...
                            if (currentIndex !== j) {
                                op.shapeMove(this.page, parent, currentIndex, parent, j);
                            }
                        }
                    }
                }
    ...
    makeStatus(symbolView: SymbolView, attri_name: string, dlt: string, isDefault: boolean) {
        ...
        const op = this.__repo.start("makeStatus");
        try {
            if (symbol instanceof SymbolUnionShape) {
                const v = isDefault ? SymbolShape.Default_State : dlt;
                const _var = new Variable(uuid(), VariableType.Status, attri_name, v);
                op.shapeAddVariable(this.page, symbol, _var);
            } else {
                const u = make_union(op, this.__document, this.page, symbol, attri_name);
                if (!u) {
    ...
    makeVar(type: VariableType, symbol: SymbolShape, name: string, values: any) {
        const op = this.__repo.start("makeVar");
        try {
    ...
    makeStateAt(union: SymbolShape, dlt: string, index?: number, hor_align?: number) {
        ...
        try {
            ...
            const op = this.__repo.start("makeStateAt");
            const copy = importSymbolShape(source, ctx); // 需要设置ctx
            const new_state = op.shapeInsert(this.__document, this.page, union, copy, idx + 1);
            modify_frame_after_inset_state(this.page, op, union);
            init_state(op, this.page, new_state as SymbolShape, dlt);
            ...
            if (_types.includes(union.type)) {
                ...
                for (let j = 0; j < sortedArr.length; j++) {
                    ...
                    if (currentIndex !== j) {
                        op.shapeMove(this.page, union, currentIndex, union, j);
                    }
                }
            }
            if (new_state) {
    ...
    extractSymbol(shapes: ShapeView[]) {
        ...
        try {
            ...
            for (let i = 0, len = actions.length; i < len; i++) {
                ...
                const ret = op.shapeInsert(this.__document, this.page, parent as GroupShape, self, insertIndex);
                op.shapeDelete(this.__document, this.page, parent as GroupShape, insertIndex + 1);
                ...
                if ([ShapeType.Artboard, ShapeType.Symbol, ShapeType.SymbolRef].includes(parent.type)) {
                    ...
                    for (let j = 0; j < sortedArr.length; j++) {
                        ...
                        if (currentIndex !== j) {
                            op.shapeMove(this.page, parent as GroupShape, currentIndex, parent as GroupShape, j);
                        }
                    }
                }
    ...
    flattenShapes(shapes: ShapeView[], name?: string) {
        ...
        try {
            ...
            for (let i = 0, len = shapes.length; i < len; i++) {
                ...
                op.shapeDelete(this.__document, this.page, p, idx);
                if (p.childs.length <= 0) {
                    deleteEmptyGroupShape(this.__document, this.page, p, op)
                }
            }
            update_frame_by_points(op, this.page, pathShape);
            const _types = [ShapeType.Artboard, ShapeType.Symbol, ShapeType.SymbolRef];
            if (_types.includes(savep.type)) {
                ...
                for (let j = 0; j < sortedArr.length; j++) {
                    ...
                    if (currentIndex !== j) {
                        op.shapeMove(this.page, savep, currentIndex, savep, j);
                    }
                }
            }
            this.__repo.commit();
    ...
    flattenBoolShape(shape: BoolShapeView): PathShape | false {
        try {
            ...
            op.shapeDelete(this.__document, this.page, parent, index);
            pathShape = op.shapeInsert(this.__document, this.page, parent, pathShape, index) as PathShape;
            const _types = [ShapeType.Artboard, ShapeType.Symbol, ShapeType.SymbolRef];
    ...
    flattenGroup(shape: ShapeView, groupname: string): PathShape | false {
        ...
        try {
            ...
            op.shapeDelete(this.__document, this.page, _parent, saveidx);
            pathShape = op.shapeInsert(this.__document, this.page, _parent, pathShape, saveidx) as PathShape;
            update_frame_by_points(op, this.page, pathShape);
            const _types = [ShapeType.Artboard, ShapeType.Symbol, ShapeType.SymbolRef];
            if (_types.includes(_parent.type)) {
                ...
                for (let j = 0; j < sortedArr.length; j++) {
                    ...
                    if (currentIndex !== j) {
                        op.shapeMove(this.page, _parent, currentIndex, _parent, j);
                    }
                }
            }
            this.__repo.commit();
    ...
    private removeContactSides(op: Operator, page: Page, shape: types.ContactShape) {
        if (shape.from) {
    ...
    private delete_inner(page: Page, _shape: ShapeView | Shape, op: Operator): boolean {
        const shape = _shape instanceof Shape ? _shape : _shape.data;
        const p = shape.parent as GroupShape;
        if (!p) return false;
        if (shape.type === ShapeType.Contact) { // 连接线删除之后需要删除两边的连接关系
            this.removeContactSides(op, page, shape as unknown as types.ContactShape);
        } else {
            this.removeContact(op, page, shape);
        }
        op.shapeDelete(this.__document, page, p as GroupShape, (p as GroupShape).indexOfChild(shape));
        if (p.childs.length <= 0 && p.type === ShapeType.Group) {
            this.delete_inner(page, p, op)
        }
        return true;
    }
    ...
    delete(shape: ShapeView): boolean {
        ...
        try {
            if (is_part_of_symbolref(shape)) {
                ...
                if (modify_variable_with_api(op, this.view, shape, VariableType.Visible, OverrideType.Visible, isVisible)) return true;
                op.shapeModifyVisible(this.page, shape.data, isVisible);
                return true;
            }
            const symbol = get_symbol_by_layer(shape);
            if (symbol) {
                clear_binds_effect(page, shape, symbol, op);
            }
            if (this.delete_inner(page.data, shape, op)) {
                if (after_remove(savep)) {
                    this.delete_inner(page.data, savep, op);
                }
                this.__repo.commit()
                return true;
            } else {
    ...
    delete_batch(shapes: ShapeView[]) {
        ...
        for (let i = 0; i < shapes.length; i++) {
            try {
                ...
                if (is_part_of_symbolref(shape)) {
                    const isVisible = !shape.isVisible;
                    if (modify_variable_with_api(op, this.view, shape, VariableType.Visible, OverrideType.Visible, isVisible)) continue;
                    op.shapeModifyVisible(this.page, shape.data, isVisible);
                    continue;
                }
                const symbol = get_symbol_by_layer(shape);
                if (symbol) {
                    clear_binds_effect(this.page, shape, symbol, op);
                }
                ...
                const savep = shape.parent as ShapeView;
                if (!savep) return false;
                this.delete_inner(page.data, shape, op);
                if (after_remove(savep)) {
                    this.delete_inner(page.data, savep, op);
                }
            } catch (error) {
    ...
    insert(parent: GroupShape, index: number, shape: Shape, adjusted = false): Shape | false {
        ...
        try {
            op.shapeInsert(this.__document, this.page, parent, shape, index);
    ...
    insertShapes(actions: { parent: GroupShape; shape: Shape; index?: number }[], adjusted = true) {
        try {
            ...
            for (const action of actions) {
                ...
                const s = op.shapeInsert(document, page, parent, shape, action.index ?? parent.childs.length);
                const _types = [ShapeType.Artboard, ShapeType.Symbol, ShapeType.SymbolRef];
                if (_types.includes(parent.type)) {
                    const Fixed = ScrollBehavior.FIXEDWHENCHILDOFSCROLLINGFRAME;
                    const sortedArr = [...parent.childs].sort((a, b) => {
                ...
                ids.push(s.id);
                const name = assign(s);
                if (name !== shape.name) op.shapeModifyName(page, s, name);
            }
            this.__repo.commit();
            return true;
        } catch (e) {
    ...
    pasteShapes1(parent: GroupShape, shapes: Shape[]): {
        ...
        try {
            ...
            for (let i = 0, len = shapes.length; i < len; i++) {
                const shape = shapes[i];
                // shape.id = uuid();
                const __shape = op.shapeInsert(this.__document, this.page, parent, shape, index);
                const name = assign(__shape);
                op.shapeModifyName(this.page, __shape, name);
        ...
        try {
            ...
            for (let i = 0, len = actions.length; i < len; i++) {
                ...
                for (let j = 0; j < shapes.length; j++) {
                    ...
                    const __shape = op.shapeInsert(this.__document, this.page, env, shapes[j], index);
                    const name = assign(__shape);
                    op.shapeModifyName(this.page, __shape, name);
    ...
    shapesModifyRadius(shapes: ShapeView[], values: number[]) {
        try {
            const op = this.__repo.start("shapesModifyRadius");
            const page = this.page;
            ...
            for (const view of shapes) {
                ...
                if (view.radiusMask) {
                    const variable = getRadiusMaskVariable(op, pageView, view, undefined);
                    if (variable) {
                        op.shapeModifyVariable(page, variable, undefined);
                    } else {
                        op.modifyRadiusMask(shape, undefined);
                    }
                }
                ...
                if (shape.radiusType === RadiusType.Rect) {
                    ...
                    if (shape instanceof SymbolRefShape) {
                        const _shape = shape4cornerRadius(op, this.view, view as SymbolRefView);
                ...
                if (needUpdateFrame) update_frame_by_points(op, this.page, shape as PathShape);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    move(shape: Shape, target: GroupShape, to: number): boolean {
        ...
        if (target.id !== parent.id || to !== index && (to + 1) !== index) { // 还是在原来位置
            const op = this.__repo.start("move");
            try {
                if (target.id === parent.id) to = index >= to ? to : to + 1;
                op.shapeMove(this.page, parent, index, target, to)
                this.__repo.commit();
                return true;
            } catch (error) {
                console.log(error)
    ...
    upperLayer(shapes: ShapeView[], step?: number) {
        ...
        try {
            const op = this.__repo.start("upperLayer");
            ...
            for (let i = 0; i < shapes.length; i++) {
                ...
                if (targetIndex !== currentIndex) {
                    adjusted = true;
                    op.shapeMove(this.page, parent, currentIndex, parent, targetIndex);
                }
    ...
    replace(document: Document, replacement: Shape[], src: Shape[]): Shape[] {
        try {
            ...
            for (let i = 0; i < src.length; i++) {
                ...
                // 先删除将被替换的图形
                const del_res = this.delete_inner(this.page, s, op);
                ...
                for (let r_i = 0; r_i < len; r_i++) { // 逐个插入replacement中的图形
                    ...
                    op.shapeInsert(this.__document, this.page, p, r, save_index);
    ...
    arrange(actions: { target: Shape, transX: number; transY: number }[]) {
        try {
            const op = this.__repo.start('arrange');
            const page = this.page;
            for (const action of actions) {
                const { target, transX, transY } = action;
                op.shapeModifyXY(page, target, target.transform.translateX + transX, target.transform.translateY + transY);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    modifyShapesX(actions: {
        ...
        try {
            const op = this.__repo.start('modifyShapesX');
            const page = this.page;
    ...
    modifyShapesY(actions: {
        ...
        try {
            const op = this.__repo.start('modifyShapesY');
            const page = this.page;
            for (let i = 0; i < actions.length; i++) {
                const action = actions[i];
                const shape = adapt2Shape(action.target)
    ...
    modifyShapesAngleCount(actions: {
        ...
        const op = this.__repo.start('modifyShapesAngleCount');
        try {
            for (let i = 0; i < actions.length; i++) {
                ...
                op.deletePoints(this.page, target, 0, target.type === ShapeType.Star ? target.counts * 2 : target.counts, 0);
                op.addPoints(this.page, target, points, 0);
                op.shapeModifyCounts(this.page, target, count);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    modifyShapesInnerAngle(actions: {
        ...
        const op = this.__repo.start('modifyShapesInnerAngle');
        try {
            for (let i = 0; i < actions.length; i++) {
                ...
                for (let index = 0; index < points.length; index++) {
                    ...
                    op.shapeModifyCurvPoint(this.page, target, index, p, 0);
                }
                op.shapeModifyInnerAngle(this.page, target, offset);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    setShapesConstrainerProportions(actions: BatchAction2[]) {
        const op = this.__repo.start('setShapesConstrainerProportions');
        try {
            for (let i = 0; i < actions.length; i++) {
                const { target, value } = actions[i];
                op.shapeModifyConstrainerProportions(this.page, adapt2Shape(target), value);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    modifyShapesWidth(shapes: ShapeView[], val: number) {
        try {
            const op = this.__repo.start('modifyShapesWidth');
            modify_shapes_width(op, this.__document, this.page, shapes, val)
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
        }
    ...
    modifyShapesHeight(shapes: ShapeView[], val: number) {
        try {
            const op = this.__repo.start('modifyShapesHeight');
            modify_shapes_height(op, this.__document, this.page, shapes, val)
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
        }
    ...
    setShapesRotate(actions: {
        ...
        try {
            const op = this.__repo.start('setShapesRotate');
            for (const action of actions) {
                const { shape: shapeView, transform } = action;
                const s = adapt2Shape(shapeView);
                op.shapeModifyRotate(this.page, s, transform);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    shapesFlip(params: {
        ...
        try {
            const op = this.__repo.start('shapesFlip');
            const page = this.page;
            for (let i = 0; i < params.length; i++) {
                const { shape, transform2 } = params[i];
                op.shapeModifyTransform(page, adapt2Shape(shape), (transform2.clone()));
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
        ...
        try {
            ...
            if (!len)
                ...
                if (bottom.parent!.id === page.id) {
                    if (bottom.mask) {
                        op.shapeModifyMask(page, bottom, false);
                        resultShapes = [bottom.id];
                    } else {
                        const gshape = newGroupShape(maskName!);
                        const saveidx = page.indexOfChild(bottom);
                        resultShapes = [group(doc, page, [bottom], gshape, page, saveidx, op).id];
                        op.shapeModifyMask(page, bottom, true);
                    }
                } else {
                    const __target_mask = !bottom.mask;
                    op.shapeModifyMask(page, bottom, __target_mask);
                    resultShapes = [bottom.id];
                }
            } else {
                const bottom = adapt2Shape(shapes[0]);
                ...
                const __mg = group(doc, page, shapes.map(shape => adapt2Shape(shape)), gshape, savep, saveidx, op);
                resultShapes = [__mg.id];
                if (!__mg.childs[0].mask) op.shapeModifyMask(page, __mg.childs[0], true);
            }
            this.__repo.commit();
            return true;
        } catch (e) {
    ...
    addShapesGradientStop(actions: { fill: Fill, stop: Stop }[]) {
        try {
            const op = this.__repo.start('addShapesGradientStop');
    ...
    deleteShapesGradientStop(actions: BatchAction5[]) {
        try {
            const op = this.__repo.start('deleteShapesGradientStop');
            for (let i = 0, l = actions.length; i < l; i++) {
                ...
                const f = type === 'fills' ? op.setFillGradient.bind(op) : op.setBorderGradient.bind(op);
                const shape = shape4fill(op, this.view, target);
            }
            this.__repo.commit();
        } catch (error) {
            console.error(error);
    ...
    setGradientOpacity(actions: { fill: Fill, opacity: number }[]) {
        try {
            const op = this.__repo.start('setGradientOpacity');
            for (let i = 0, l = actions.length; i < l; i++) {
                const { fill, opacity } = actions[i];
                const gradient = fill.gradient;
                if (!gradient) continue;
                op.setGradientOpacity(gradient, opacity);
            }
            this.__repo.commit();
        } catch (error) {
    ...
    setShapesFillAsImage(actions: {
        ...
        try {
            const op = this.__repo.start('setShapesFillAsImage');
            const document = this.__document;
            for (const action of actions) {
                const { shape, ref, media, width, height } = action;
                const target = shape4fill(op, this.view, shape);
                ...
                if (fills instanceof Variable) {
                    ...
                    if (!fills.value[index].imageScaleMode) {
                        op.setFillScaleMode(fills.value[index], types.ImageScaleMode.Fill);
                    }
                    if (fills.value[index].fillType !== FillType.Pattern) {
                        op.setFillType(fills.value[index], FillType.Pattern);
                    }
                    op.setFillImageRef(document, fills.value[index], ref, media);
                    op.setFillImageOriginWidth(fills.value[index], width);
                    op.setFillImageOriginHeight(fills.value[index], height);
                } else {
                    ...
                    if (index < 0) {
                        ...
                        op.addFillAt(fills, fill, 0);
                    } else {
                        if (fills[index].fillType !== FillType.Pattern) {
                            op.setFillType(fills[index], FillType.Pattern);
                        }
                        op.setFillImageRef(document, fills[index], ref, media);
                        op.setFillImageOriginWidth(fills[index], width);
                        op.setFillImageOriginHeight(fills[index], height);
                    }
                }
            }
            this.__repo.commit();
    ...
    setShapeBorderFillExchange(shapes: ShapeView[]) {
        try {
            const op = this.__repo.start('setShapeBorderFillExchange');
            for (let i = 0; i < shapes.length; i++) {
                const shape = shapes[i];
                const fillMask = shape.fillsMask;
                const borderFillMask = shape.borderFillsMask;

                const fillShape = shape4fill(op, this.view, shape);
                const borderShape = shape4border(op, this.view, shape);
                ...
                const variableFill = getFillMaskVariable(op, this.view, shape, fillMask);
                const variableBorder = getBorderMaskVariable(op, this.view, shape, borderFillMask);

                if (borderFillMask) {
                    if (variableFill) {
                        op.shapeModifyVariable(this.page, variableFill, borderFillMask);
                        if (!fillMask && variableBorder) op.shapeModifyVariable(this.page, variableBorder, undefined);
                    } else {
                        op.modifyFillsMask(this.page, adapt2Shape(shape), borderFillMask);
                        if (!fillMask) op.setBorderFillMask(adapt2Shape(shape).style, undefined);
                    }
                } else {
                    op.deleteFills(fills, 0, fills.length);
                    op.addFills(fills, borderFills.map(i => importFill(i)));
                }
                if (fillMask) {
                    if (variableBorder) {
                        op.shapeModifyVariable(this.page, variableBorder, fillMask);
                        if (!borderFillMask && variableFill) op.shapeModifyVariable(this.page, variableFill, undefined);
                    } else {
                        op.setBorderFillMask(adapt2Shape(shape).style, fillMask);
                        if (!borderFillMask) op.modifyFillsMask(this.page, adapt2Shape(shape), undefined);
                    }
                } else {
                    op.deleteFills(borderFills, 0, borderFills.length);
                    op.addFills(borderFills, fills.map(i => importFill(i)));
                }
            }
            this.__repo.commit();
        } catch (error) {
    ...
    setShapesMarkerType(actions: BatchAction2[]) {
        const op = this.__repo.start('setShapesMarkerType');
        try {
            for (let i = 0; i < actions.length; i++) {
                const { target, value } = actions[i];
                if (modify_variable_with_api(op, this.view, target, VariableType.MarkerType, value.isEnd ? OverrideType.EndMarkerType : OverrideType.StartMarkerType, value.mt)) continue;
                if (value.isEnd) {
                    op.shapeModifyEndMarkerType(this.page, adapt2Shape(target), value.mt);
                } else {
                    op.shapeModifyStartMarkerType(this.page, adapt2Shape(target), value.mt);
                }
            }
            this.__repo.commit();
        } catch (error) {
    ...
    setShapesEndpoint(actions: BatchAction2[]) {
        const op = this.__repo.start('setShapesEndpoint');
        try {
            for (let i = 0; i < actions.length; i++) {
                const { target, value } = actions[i];
                if (modify_variable_with_api(op, this.view, target, VariableType.MarkerType, OverrideType.StartMarkerType, value.mt)) {
                    modify_variable_with_api(op, this.view, target, VariableType.MarkerType, OverrideType.EndMarkerType, value.mt)
                    continue;
                }
                op.shapeModifyEndMarkerType(this.page, adapt2Shape(target), value);
                op.shapeModifyStartMarkerType(this.page, adapt2Shape(target), value);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    exchangeShapesMarkerType(actions: BatchAction2[]) {
        const op = this.__repo.start('exchangeShapesMarkerType');
        try {
            for (let i = 0; i < actions.length; i++) {
                const { target, value } = actions[i];
                ...
                if (modify_variable_with_api(op, this.view, target, VariableType.MarkerType, OverrideType.EndMarkerType, startMarkerType || MarkerType.Line)) {
                    modify_variable_with_api(op, this.view, target, VariableType.MarkerType, OverrideType.StartMarkerType, endMarkerType || MarkerType.Line)
                    continue;
                }
                op.shapeModifyEndMarkerType(this.page, adapt2Shape(target), startMarkerType || MarkerType.Line);
                op.shapeModifyStartMarkerType(this.page, adapt2Shape(target), endMarkerType || MarkerType.Line);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    shapesExportFormatUnify(actions: ExportFormatReplaceAction[]) {
        try {
            const op = this.__repo.start('shapesExportFormatUnify');
            for (let i = 0; i < actions.length; i++) {
                const { target, value } = actions[i];
                if (target.exportOptions) {
                    op.deleteExportFormats(this.page, target, 0, target.exportOptions.exportFormats.length);
                }
                op.addExportFormats(this.page, target, value);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    setPageExportPreviewUnfold(unfold: boolean) {
        try {
            const op = this.__repo.start('setPageExportPreviewUnfold');
            op.setPageExportPreviewUnfold(this.__document, this.page.id, unfold);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    shapesDeleteExportFormat(actions: ExportFormatDeleteAction[]) {
        try {
            const op = this.__repo.start('shapesDeleteExportFormat');
            for (let i = 0; i < actions.length; i++) {
                const { target, index } = actions[i];
                op.deleteExportFormatAt(this.page, target, index);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    pageDeleteExportFormat(idx: number) {
        try {
            ...
            if (format) {
                const op = this.__repo.start('pageDeleteExportFormat');
                op.deletePageExportFormatAt(this.page, idx);
                this.__repo.commit();
            }
        } catch (error) {
            this.__repo.rollback();
    ...
    setShapesExportFormatScale(actions: ExportFormatScaleAction[]) {
        try {
            const op = this.__repo.start('setShapesExportFormatScale');
            for (let i = 0; i < actions.length; i++) {
                const { target, index, value } = actions[i];
                op.setExportFormatScale(this.page, target, index, value);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    setPageExportFormatScale(idx: number, scale: number) {
        try {
            const op = this.__repo.start('setPageExportFormatScale');
            op.setPageExportFormatScale(this.page, idx, scale);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    setShapesExportFormatName(actions: ExportFormatNameAction[]) {
        try {
            const op = this.__repo.start('setShapesExportFormatName');
            for (let i = 0; i < actions.length; i++) {
                const { target, index, value } = actions[i];
                op.setExportFormatName(this.page, target, index, value);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    setPageExportFormatName(idx: number, name: string) {
        try {
            const op = this.__repo.start('setPageExportFormatName');
            op.setPageExportFormatName(this.page, idx, name);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    setShapesExportFormatPrefix(actions: ExportFormatPrefixAction[]) {
        try {
            const op = this.__repo.start('setShapesExportFormatPerfix');
            for (let i = 0; i < actions.length; i++) {
                const { target, index, value } = actions[i];
                op.setExportFormatPerfix(this.page, target, index, value);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    setPageExportFormatPrefix(idx: number, name: ExportFormatNameingScheme) {
        try {
            const op = this.__repo.start('setPageExportFormatPerfix');
            op.setPageExportFormatPerfix(this.page, idx, name);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    setShapesExportFormatFileFormat(actions: ExportFormatFileFormatAction[]) {
        try {
            const op = this.__repo.start('setShapesExportFormatFileFormat');
            for (let i = 0; i < actions.length; i++) {
                const { target, index, value } = actions[i];
                op.setExportFormatFileFormat(this.page, target, index, value);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    setPageExportFormatFileFormat(idx: number, name: ExportFileFormat) {
        try {
            const op = this.__repo.start('setPageExportFormatFileFormat');
            op.setPageExportFormatFileFormat(this.page, idx, name);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    setPrototypeStart(shape: ShapeView, startpoint: PrototypeStartingPoint) {
        try {
            const op = this.__repo.start('setPrototypeStart');
            const __shape = adapt2Shape(shape);
            op.setShapeProtoStart(this.page, __shape, startpoint);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    delPrototypeStart(shape: ShapeView) {
        try {
            const op = this.__repo.start('delPrototypeStart');
            const __shape = adapt2Shape(shape);
            op.setShapeProtoStart(this.page, __shape, undefined);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    private shape4protoActions(op: Operator, shape: ShapeView, id: string | undefined) {
        const _var = prepareVar(op, this.view, shape, OverrideType.ProtoInteractions, VariableType.ProtoInteractions, (_var) => {
        ...
        if (_var && id && !(_var.var.value as PrototypeInteraction[]).find(v => v.id === id)) {
            ...
            if (i) {
                ...
                op.insertShapeprototypeInteractions(this.page, _var.var, a);
            }
        }
    ...
    insertPrototypeAction(shape: ShapeView, action: PrototypeInteraction) {
        try {
            const op = this.__repo.start('insertPrototypeAction');
            const _shape = this.shape4protoActions(op, shape, undefined);
            op.insertShapeprototypeInteractions(this.page, _shape, action);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    deletePrototypeAction(shape: ShapeView, id: string) {
        try {
            const op = this.__repo.start('deletePrototypeAction');
            const _shape = this.shape4protoActions(op, shape, id);
            if (_shape instanceof Variable) {
                op.shapeModifyPrototypeActionDeleted(this.page, _shape, id, true);
            } else {
                op.deleteShapePrototypeInteractions(this.page, _shape, id);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    setPrototypeActionEvent(shape: ShapeView, id: string, value: PrototypeEvents) {
        try {
            const op = this.__repo.start('setPrototypeActionEvent');
            const _shape = this.shape4protoActions(op, shape, id);
            op.shapeModifyPrototypeActionEvent(this.page, _shape, id, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    setPrototypeActionEventTime(shape: ShapeView, id: string, value: number) {
        try {
            const op = this.__repo.start('setPrototypeActionEventTime');
            const _shape = this.shape4protoActions(op, shape, id);
            op.shapeModifyPrototypeActionEventTime(this.page, _shape, id, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    setPrototypeActionConnNav(shape: ShapeView, id: string, conn: PrototypeConnectionType | undefined, nav: PrototypeNavigationType | undefined) {
        try {
            const op = this.__repo.start('setPrototypeActionConnectionType');
            const __shape = this.shape4protoActions(op, shape, id);
            ...
            op.shapeModifyPrototypeActionConnNav(this.page, __shape, id, conn, nav);
            ...
            if (nav === PrototypeNavigationType.SCROLLTO && old_nav !== PrototypeNavigationType.SCROLLTO) {
                ...
                if (!arr.includes(transitionType)) {
                    op.shapeModifyPrototypeActionTransitionType(this.page, __shape, id, PrototypeTransitionType.INSTANTTRANSITION)
                }
                op.shapeModifyPrototypeActionTargetNodeID(this.page, __shape, id, undefined)
            }
    ...
    setPrototypeActionTargetNodeID(shape: ShapeView, id: string, value: string) {
        try {
            const op = this.__repo.start('setPrototypeActionTargetNodeID');
            const __shape = this.shape4protoActions(op, shape, id);
            op.shapeModifyPrototypeActionTargetNodeID(this.page, __shape, id, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    setPrototypeActionTransitionType(shape: ShapeView, id: string, value: PrototypeTransitionType) {
        try {
            const op = this.__repo.start('setPrototypeActionTransitionType');
            const __shape = this.shape4protoActions(op, shape, id);
            op.shapeModifyPrototypeActionTransitionType(this.page, __shape, id, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    setPrototypeActionTransitionDuration(shape: ShapeView, id: string, value: number) {
        try {
            const op = this.__repo.start('setPrototypeActionTransitionDuration');
            const __shape = this.shape4protoActions(op, shape, id);
            op.shapeModifyPrototypeActionTransitionDuration(this.page, __shape, id, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    setPrototypeActionEasingType(shape: ShapeView, id: string, value: PrototypeEasingType, esfn: PrototypeEasingBezier) {
        try {
            const op = this.__repo.start('setPrototypeActionEasingType');
            const __shape = this.shape4protoActions(op, shape, id);
            ...
            if (!action) return;
            op.shapeModifyPrototypeActionEasingType(this.page, __shape, id, value);
            let val = action.easingFunction
            if (value === PrototypeEasingType.CUSTOMCUBIC) {
                if (val) {
                    op.shapeModifyPrototypeActionEasingFunction(this.page, __shape, id, val)
                } else {
                    op.shapeModifyPrototypeActionEasingFunction(this.page, __shape, id, esfn)
                }
            } else {
                op.shapeModifyPrototypeActionEasingFunction(this.page, __shape, id, esfn)
            }
    ...
    setPrototypeActionConnectionURL(shape: ShapeView, id: string, value: string) {
        try {
            const op = this.__repo.start('setPrototypeActionConnectionURL');
            const __shape = this.shape4protoActions(op, shape, id);
            op.shapeModifyPrototypeActionConnectionURL(this.page, __shape, id, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    setPrototypeIsOpenNewTab(shape: ShapeView, id: string, value: boolean) {
        try {
            const op = this.__repo.start('setPrototypeIsOpenNewTab');
            const __shape = this.shape4protoActions(op, shape, id);
            op.shapeModifyPrototypeIsOpenNewTab(this.page, __shape, id, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    setPrototypeActionOpenUrlInNewTab(shape: ShapeView, id: string, value: boolean) {
        try {
            const op = this.__repo.start('setPrototypeActionOpenUrlInNewTab');
            const __shape = this.shape4protoActions(op, shape, id);
            op.shapeModifyPrototypeActionOpenUrlInNewTab(this.page, __shape, id, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    setPrototypeActionEasingFunction(shape: ShapeView, id: string, value: PrototypeEasingBezier) {
        try {
            const op = this.__repo.start('setPrototypeActionEasingFunction');
            const __shape = this.shape4protoActions(op, shape, id);
            op.shapeModifyPrototypeActionEasingFunction(this.page, __shape, id, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    setPrototypeExtraScrollOffsetX(shape: ShapeView, id: string, value: number) {
        try {
            const op = this.__repo.start('setPrototypeExtraScrollOffsetX');
            const __shape = this.shape4protoActions(op, shape, id);
            op.shapeModifyPrototypeExtraScrollOffsetX(this.page, __shape, id, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    setPrototypeExtraScrollOffsetY(shape: ShapeView, id: string, value: number) {
        try {
            const op = this.__repo.start('setPrototypeExtraScrollOffsetY');
            const __shape = this.shape4protoActions(op, shape, id);
            op.shapeModifyPrototypeExtraScrollOffsetY(this.page, __shape, id, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
            throw error;
    ...
    setOverlayPositionType(shape: ShapeView, value: OverlayPositionType) {
        try {
            const op = this.__repo.start('setOverlayPositionType');
            const __shape = adapt2Shape(shape);
            op.shapeModifyOverlayPositionType(this.page, __shape, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
        }
    ...
    setOverlayPositionTypeMarginTop(shape: ShapeView, value: number) {
        try {
            const op = this.__repo.start('setOverlayPositionTypeMarginTop');
            const __shape = adapt2Shape(shape);
            op.shapeModifyOverlayPositionTypeMarginTop(this.page, __shape, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
        }
    ...
    setOverlayPositionTypeMarginBottom(shape: ShapeView, value: number) {
        try {
            const op = this.__repo.start('setOverlayPositionTypeMarginBottom');
            const __shape = adapt2Shape(shape);
            op.shapeModifyOverlayPositionTypeMarginBottom(this.page, __shape, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
        }
    ...
    setOverlayPositionTypeMarginLeft(shape: ShapeView, value: number) {
        try {
            const op = this.__repo.start('setOverlayPositionTypeMarginLeft');
            const __shape = adapt2Shape(shape);
            op.shapeModifyOverlayPositionTypeMarginLeft(this.page, __shape, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
        }
    ...
    setOverlayPositionTypeMarginRight(shape: ShapeView, value: number) {
        try {
            const op = this.__repo.start('setOverlayPositionTypeMarginRight');
            const __shape = adapt2Shape(shape);
            op.shapeModifyOverlayPositionTypeMarginRight(this.page, __shape, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
        }
    ...
    setOverlayBackgroundInteraction(shape: ShapeView, value: OverlayBackgroundInteraction) {
        try {
            const op = this.__repo.start('setOverlayBackgroundInteraction');
            const __shape = adapt2Shape(shape);
            op.shapeModifyOverlayBackgroundInteraction(this.page, __shape, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
        }
    ...
    setOverlayBackgroundAppearance(shape: ShapeView, value?: OverlayBackgroundAppearance) {
        try {
            const op = this.__repo.start('setOverlayBackgroundAppearance');
            const __shape = adapt2Shape(shape);
            op.shapeModifyOverlayBackgroundAppearance(this.page, __shape, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
        }
    ...
    setScrollDirection(shape: ShapeView, value: ScrollDirection) {
        try {
            const op = this.__repo.start('setScrollDirection');
            const __shape = adapt2Shape(shape);
            op.shapeModifyscrollDirection(this.page, __shape, value);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
        }
    ...
    setScrollBehavior(shapes: ShapeView[], value: ScrollBehavior) {
        const op = this.__repo.start('setScrollBehavior');
        try {
            ...
            for (let i = 0; i < shapes.length; i++) {
                ...
                if (value === Fixed && __shape.scrollBehavior !== Fixed) {
                    ...
                    op.shapeMove(this.page, parent, currentIndex, parent, fixed_index);
                }

                op.shapeModifyScrollBehavior(this.page, __shape, value);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    toggleShapesVisible(shapes: ShapeView[]) {
        const op = this.__repo.start('setShapesVisible');
        try {
            for (let i = 0; i < shapes.length; i++) {
                let shape: ShapeView = shapes[i];
                if (!shape) continue;
                const isVisible = !shape.isVisible;
                if (modify_variable_with_api(op, this.view, shape, VariableType.Visible, OverrideType.Visible, isVisible)) {
                    continue;
                }
                op.shapeModifyVisible(this.page, shape.data, isVisible);
            }
    ...
    toggleShapesLock(shapes: ShapeView[]) {
        const op = this.__repo.start('setShapesLocked');
        try {
            for (let i = 0; i < shapes.length; i++) {
                let shape: ShapeView | undefined = shapes[i];
                const isLocked = !shape.isLocked;
                if (modify_variable_with_api(op, this.view, shape, VariableType.Lock, OverrideType.Lock, isLocked)) {
                    continue;
                }
                op.shapeModifyLock(this.page, shape.data, isLocked);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    setBackground(color: Color) {
        const op = this.__repo.start('setBackground');
        try {
            op.pageModifyBackground(this.__document, this.page.id, color);
            this.__repo.commit();
        } catch (error) {
            console.log(error);
            this.__repo.rollback();
    ...
    setShapesRadius(shapes: Shape[], lt: number, rt: number, rb: number, lb: number) {
        const op = this.__repo.start('setShapesRadius');
        try {
            for (let i = 0; i < shapes.length; i++) {
                const s = shapes[i];
                op.shapeModifyRadius(this.page, s as RectShape, lt, rt, rb, lb);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
    ...
    afterShapeListDrag(shapes: Shape[], host: Shape, position: 'upper' | 'inner' | 'lower') {
        ...
        try {
            const operator = this.__repo.start('afterShapeListDrag');
            if (position === "inner") {
                for (let i = 0, l = pre.length; i < l; i++) {
                    ...
                    if (parent.id === host.id) { // 同一父级
                        ...
                        if (host instanceof BoolShape) {
                            ...
                            operator.shapeModifyBoolOp(this.page, item, op);
                        }
                    }

                    operator.shapeMove(this.page, parent, parent.indexOfChild(item), host as GroupShape, last);

                    translateTo(operator, this.page, item, beforeXY.x, beforeXY.y);

                    if (after_remove(parent)) {
                        this.delete_inner(this.page, parent, operator);
                    }
                    const _types = [ShapeType.Artboard, ShapeType.Symbol, ShapeType.SymbolRef];
                    if (_types.includes(host.type)) {
                        ...
                        for (let j = 0; j < sortedArr.length; j++) {
                            ...
                            if (currentIndex !== j) {
                                operator.shapeMove(this.page, host as GroupShape, currentIndex, host as GroupShape, j);
                            }
                        }
                    }
                }
                ...
                for (let i = 0, l = pre.length; i < l; i++) {
                    ...
                    operator.shapeMove(this.page, parent, parent.indexOfChild(item), host_parent, index);
                    ...
                    if (_types.includes(host_parent.type)) {
                        ...
                        for (let j = 0; j < sortedArr.length; j++) {
                            ...
                            if (currentIndex !== j) {
                                operator.shapeMove(this.page, host_parent, currentIndex, host_parent, j);
                            }
                        }
                    }
                }
    ...
    setLinesLength(shapes: Shape[], v: number) {
        const op = this.__repo.start('setLinesLength');
        try {
            for (let i = 0, len = shapes.length; i < len; i++) {
                const s = shapes[i];
                ...
                op.shapeModifyWH(this.page, s, v * Math.cos(r), v * Math.sin(r));
                const o2 = s.matrix2Root().computeCoord2(0, 0);
                translate(op, this.page, s, o1.x - o2.x, o1.y - o2.y);
            }
            this.__repo.commit();
        } catch (error) {
            console.log(error);
    ...
    modifyStyleByEyeDropper(shapes: ShapeView[], color: Color) {
        try {
            const op = this.__repo.start('modifyStyleByEyeDropper');
            const page = this.page;
            for (let i = 0; i < shapes.length; i++) {
                const shape = adapt2Shape(shapes[i]);
                if (shape.isVirtualShape) continue;
                ...
                if (shape.type === ShapeType.Text) {
                    ...
                    op.textModifyColor(page, __textShape, 0, __textShape.text.length, _color);
                    continue;
                }
                const style = shape.style;
                if (style.fills.length) {
                    const s = shape4fill(op, this.view, shapes[i]);
                    const fills = s instanceof Variable ? s.value : s.getFills();
                    op.setFillColor(fills[fills.length - 1], _color);
                    continue;
                }
                if (style.borders && style.borders.strokePaints.length) {
                    const s = shape4border(op, this.view, shapes[i]);
    ...
    pasteProperties(shapes: ShapeView[], source: any) {
        try {
            const op = this.__repo.start('pasteProperties');
            const page = this.page;
            ...
            for (const view of flatten) {
                const fillShape = shape4fill(op, this.view, view);
                const borderShape = shape4border(op, this.view, view);
                ...
                op.deleteFills(v_fills, 0, v_fills.length);
                op.addFills(v_fills, fills.map(i => importFill(i, ctx)));
                op.deleteFills(v_border.strokePaints, 0, v_border.strokePaints.length);
                op.addFills(v_border.strokePaints, borders.strokePaints.map(i => importFill(i, ctx)));
                op.setBorderSide(v_border, borders.sideSetting);
                op.setBorderPosition(v_border, borders.position);

                const variableFill = getFillMaskVariable(op, this.view, view, fillsMask);
                const variableBorder = getBorderMaskVariable(op, this.view, view, bordersMask);
                const variableBorderFill = getBorderFillMaskVariable(op, this.view, view, borderFillsMask);
                if (variableFill) {
                    op.shapeModifyVariable(this.page, variableFill, fillsMask);
                } else {
                    op.modifyFillsMask(this.page, adapt2Shape(view), fillsMask);
                }
                if (variableBorder) {
                    op.shapeModifyVariable(this.page, variableBorder, bordersMask);
                } else {
                    op.modifyBorderMask(adapt2Shape(view).style, bordersMask);
                }
                if (variableBorderFill) {
                    op.shapeModifyVariable(this.page, variableBorderFill, borderFillsMask);
                } else {
                    op.setBorderFillMask(adapt2Shape(view).style, borderFillsMask);
                }
            }
            for (let i = 0; i < shapes.length; i++) {
                const view = shapes[i];
                ...
                if (shape.isVirtualShape) continue;
                // shadows
                const shadowShadow = shape4shadow(op, this.view, view);
                const v_shadows = shadowShadow instanceof Shape ? shadowShadow.style.shadows : shadowShadow.value as BasicArray<Shadow>;
                op.deleteShadows(v_shadows, 0, v_shadows.length);
                op.addShadows(v_shadows, shadows.map((i) => importShadow(i)));
                const variableShadow = getShadowMaskVariable(op, this.view, view, shadowsMask);
                if (variableShadow) {
                    op.shapeModifyVariable(this.page, variableShadow, shadowsMask);
                } else {
                    op.modifyShadowsMask(this.page, adapt2Shape(view), shadowsMask);
                }
                // blur
                const blurShape = shape4blur(op, view, this.view);
                op.deleteBlur(shape.style);
                if (blur) {
                    if (blurShape instanceof Shape) {
                        op.addBlur(shape.style, importBlur(blur));
                    } else {
                        op.shapeModifyVariable(this.page, blurShape, importBlur(blur));
                    }
                }
                const variableBlur = getBlurMaskVariable(op, this.view, view, blurMask);
                if (variableBlur) {
                    op.shapeModifyVariable(this.page, variableBlur, blurMask);
                } else {
                    op.modifyBlurMask(this.page, adapt2Shape(view), blurMask);
                }
                // radius
                const variableRadius = getRadiusMaskVariable(op, this.__page, shapes[i], radiusMask);
                if (variableRadius) {
                    op.shapeModifyVariable(page, variableRadius, radiusMask);
                } else {
                    op.modifyRadiusMask(shape, radiusMask);
                }
                let needUpdateFrame = false;
                if (shape instanceof SymbolRefShape) {
                    const _shape = shape4cornerRadius(op, this.view, shapes[i] as SymbolRefView);
                    ...
                    if (isRect) {
                        ...
                        shape.pathsegs.forEach((seg, index) => {
                            for (let _i = 0; _i < seg.points.length; _i++) {
                                ...
                                op.modifyPointCornerRadius(page, shape, _i, radius[0], index);
                                needUpdateFrame = true;
                            }
                        });
                    }
                } else {
                    op.shapeModifyFixedRadius(page, shape as GroupShape | TextShape, radius[0]);
                }

                if (needUpdateFrame && !((shape instanceof StarShape || shape instanceof PolygonShape) && !shape.haveEdit)) {
                    update_frame_by_points(op, this.page, shape as PathShape);
                }

                // contextSetting
                const csShape = shape4contextSettings(op, view, this.view);
                op.shapeModifyContextSettingsOpacity(page, csShape, contextSetting?.opacity ?? 1);
                op.shapeModifyContextSettingsBlendMode(page, csShape, contextSetting?.blenMode ?? types.BlendMode.Normal);
                // mark
                if (mark?.start) {
                    const __start = importMarkerType(mark.start);
                    op.shapeModifyStartMarkerType(page, shape, __start);
                }
                if (mark?.end) {
                    const __end = importMarkerType(mark.end);
                    op.shapeModifyEndMarkerType(page, shape, __end);
                }
                // text
                if (text && shape instanceof TextShape) {
                    ...
                    if (attr) {
                        op.shapeModifyTextVerAlign(page, __view, attr.verAlign!); // 垂直位置
                        op.shapeModifyTextBehaviour(page, __view.text, attr.textBehaviour!); // 宽高表现
                    }
                    if (alpha) {
                        op.textModifyColor(page, __view, 0, len, alpha.color); // 字体颜色
                        op.textModifyFontName(page, __view, 0, len, alpha.fontName!); // 字体
                        op.textModifyFontSize(page, __view, 0, len, alpha.fontSize!); // 字号
                        op.textModifyItalic(page, __view, !!alpha.italic, 0, len); // 斜体
                        op.textModifyKerning(page, __view, alpha.kerning || 0, 0, len); // 字间距
                        op.textModifyUnderline(page, __view, alpha.underline, 0, len); // 下划线
                        op.textModifyStrikethrough(page, __view, alpha.strikethrough, 0, len); // 删除线
                        op.textModifyWeight(page, __view, alpha.weight!, 0, len); // 字重
                        op.textModifyHighlightColor(page, __view, 0, len, alpha.highlight); // 高亮底色
                        needFixFrame = true;
                    }

                    const alphaAttr = __text.paras[0]?.attr;
                    if (alphaAttr) {
                        op.textModifyParaSpacing(page, __view, alphaAttr.paraSpacing || 0, 0, len); // 段落间距
                        op.textModifyAutoLineHeight(page, __view, alphaAttr.autoLineHeight ?? true, 0, len)
                        op.textModifyMinLineHeight(page, __view, alphaAttr.minimumLineHeight!, 0, len); // 行高
                        op.textModifyMaxLineHeight(page, __view, alphaAttr.maximumLineHeight!, 0, len); // 行高
                        op.textModifyHorAlign(page, __view, alphaAttr.alignment!, 0, len); // 水平位置
                        needFixFrame = true;
                    }

                    needFixFrame && fixTextShapeFrameByLayout(op, page, __view);
                }
            }
            this.__repo.commit();
        } catch (error) {
        ...
        try {
            ...
            for (const view of __shapes) {
                ...
                if (view instanceof TextShapeView) {
                    ...
                    op.shapeDelete(document, page, parent, index);
                    pathShape = op.shapeInsert(document, page, parent, pathShape, index) as PathShape;
                    update_frame_by_points(op, page, pathShape);
                    ...
                    const border2shape = (border: Border) => {
                        ...
                        pathshape.scrollBehavior = shape.scrollBehavior;
                        const index = parent.indexOfChild(shape);
                        pathshape = op.shapeInsert(document, page, parent, pathshape, index + 1) as PathShape;
                        update_frame_by_points(op, page, pathshape);
                        ids.push(pathshape.id);
                    }
                    border2shape(borders);
                    if (shape.style.fills.length) {
                        op.deleteStrokePaints(page, shape, 0, borders.strokePaints.length);
                        ids.push(shape.id);
                        if (shape instanceof PathShape) update_frame_by_points(op, page, shape);
                    } else {
                        op.shapeDelete(document, page, parent, parent.indexOfChild(shape));
                    }
                }
            }
            this.__repo.commit();
        ...
        try {
            ...
            for (const item of images) {
                ...
                if ((item.pack as ImagePack).size) {
                    ...
                    const __s = op.shapeInsert(document, page, parent, shape, targetIndex);
                    if (__s) {
                        ids.push(__s.id);
    ...
    uniformScale(units: UniformScaleUnit[], ratio: number) {
        try {
            const op = this.__repo.start('uniformScale');
            uniformScale(op, this.page, units, ratio);
            this.__repo.commit();
        } catch (e) {
            this.__repo.rollback();
            console.error(e);
    ...
    modifyShapesStartingAngle(shapes: ShapeView[], value: number) {
        try {
            const op = this.__repo.start('modifyShapesStartAngle');
            modifyStartingAngle(op, this.page, shapes, value);
            this.__repo.commit();
        } catch (e) {
            this.__repo.rollback();
            console.error(e);
    ...
    modifyShapesSweep(shapes: ShapeView[], value: number) {
        try {
            const op = this.__repo.start('modifyShapesSweep');
            modifySweep(op, this.page, shapes, value);
            this.__repo.commit();
        } catch (e) {
            this.__repo.rollback();
            console.error(e);
    ...
    modifyShapesRadius(shapes: ShapeView[], value: number) {
        try {
            const op = this.__repo.start('modifyShapesRadius');
            modifyRadius(op, this.page, shapes, value);
            this.__repo.commit();
        } catch (e) {
            this.__repo.rollback();
            console.error(e);
    ...
    modifyContainersFrameMaskStatus(shapes: ShapeView[], value: boolean) {
        try {
            const op = this.__repo.start('modifyContainersFrameMaskStatus');
            const page = this.page;
            for (const view of shapes) {
                if (modify_variable_with_api(op, this.__page, view, VariableType.FrameMaskDisabled, OverrideType.FrameMaskDisabled, value)) {
                    continue;
                }
                if (view.isVirtualShape) continue;
                if (view instanceof ArtboardView || view instanceof SymbolView || view instanceof SymbolRefView) {
                    const shape = adapt2Shape(view);
                    op.modifyContainersFrameMaskStatus(page, shape, value);
                }
            }
            this.__repo.commit();
        } catch (e) {
    ...
    tidyUpShapesLayout(shape_rows: ShapeView[][], hor: number, ver: number, dir: boolean, algin: TidyUpAlign) {
        const op = this.__repo.start('tidyUpShapesLayout');
        try {
            tidyUpLayout(this.page, op, shape_rows, hor, ver, dir, algin);
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
        }
    ...
    deleteAutoLayout(shapes: ShapeView[]) {
        const op = this.__repo.start("deleteAutoLayout");
        try {
            for (let i = 0, len = shapes.length; i < len; i++) {
                ...
                for (let i = 0; i < childs.length; i++) {
                    ...
                    op.shapeModifyXY(this.page, adapt2Shape(child), child.transform.translateX, child.transform.translateY);
                }
                const w = view.frame.width;
                const h = view.frame.height;
                expandTo(op, this.__document, this.page, adapt2Shape(view), w, h);
                const shape = shape4Autolayout(op, shapes[i], this.view);
                op.shapeAutoLayout(this.page, shape, undefined);
            }
            this.__repo.commit();
        } catch (error) {
            console.log(error);
    ...
    modifyAutoLayoutPadding(shapes: ShapeView[], padding: number, direction: PaddingDir) {
        const op = this.__repo.start("modifyAutoLayoutPadding");
        try {
            for (let i = 0, len = shapes.length; i < len; i++) {
                const shape = shape4Autolayout(op, shapes[i], this.view);
                op.shapeModifyAutoLayoutPadding(this.page, shape, Math.max(padding, 0), direction);
            }
            this.__repo.commit();
        } catch (e) {
            console.error(e);
    ...
    modifyAutoLayoutHorPadding(shapes: ShapeView[], hor: number, right: number) {
        const op = this.__repo.start("modifyAutoLayoutHorPadding");
        try {
            for (let i = 0, len = shapes.length; i < len; i++) {
                const shape = shape4Autolayout(op, shapes[i], this.view);
                op.shapeModifyAutoLayoutHorPadding(this.page, shape, Math.max(hor, 0), Math.max(right, 0));
            }
            this.__repo.commit();
        } catch (e) {
            console.error(e);
    ...
    modifyAutoLayoutVerPadding(shapes: ShapeView[], ver: number, bottom: number) {
        const op = this.__repo.start("modifyAutoLayoutVerPadding");
        try {
            for (let i = 0, len = shapes.length; i < len; i++) {
                const shape = shape4Autolayout(op, shapes[i], this.view);
                op.shapeModifyAutoLayoutVerPadding(this.page, shape, Math.max(ver, 0), Math.max(bottom, 0));
            }
            this.__repo.commit();
        } catch (e) {
            console.error(e);
    ...
    modifyAutoLayoutDispersed(shapes: ShapeView[], wrap: types.StackWrap, mode: types.StackMode) {
        const op = this.__repo.start("modifyAutoLayoutDispersed");
        try {
            for (let i = 0, len = shapes.length; i < len; i++) {
                const shape = shape4Autolayout(op, shapes[i], this.view);
                op.shapeModifyAutoLayoutWrap(this.page, shape, wrap);
                op.shapeModifyAutoLayoutMode(this.page, shape, mode);
            }
            this.__repo.commit();
        } catch (e) {
            console.error(e);
    ...
    modifyAutoLayoutSpace(shapes: ShapeView[], space: number, direction: PaddingDir) {
        const op = this.__repo.start("modifyAutoLayoutSpace");
        try {
            for (let i = 0, len = shapes.length; i < len; i++) {
                const shape = shape4Autolayout(op, shapes[i], this.view);
                op.shapeModifyAutoLayoutSpace(this.page, shape, space, direction);
                op.shapeModifyAutoLayoutGapSizing(this.page, shape, types.StackSizing.Fixed, direction);
            }
            this.__repo.commit();
        } catch (e) {
            console.error(e);
    ...
    modifyAutoLayoutAlignItems(shapes: ShapeView[], primary: types.StackAlign, counter: types.StackAlign) {
        const op = this.__repo.start("modifyAutoLayoutAlignItems");
        try {
            for (let i = 0, len = shapes.length; i < len; i++) {
                const shape = shape4Autolayout(op, shapes[i], this.view);
                op.shapeModifyAutoLayoutAlignItems(this.page, shape, primary, counter);
            }
            this.__repo.commit();
        } catch (e) {
            console.error(e);
    ...
    modifyAutoLayoutSizing(shapes: ShapeView[], sizing: types.StackSizing, direction: PaddingDir) {
        const op = this.__repo.start("modifyAutoLayoutSizing");
        try {
            for (let i = 0, len = shapes.length; i < len; i++) {
                const view = shapes[i];
                const shape = shape4Autolayout(op, view, this.view);
                op.shapeModifyAutoLayoutSizing(this.page, shape, sizing, direction);
                op.shapeModifyAutoLayoutGapSizing(this.page, shape, types.StackSizing.Fixed, direction);
    ...
    modifyAutoLayoutGapSizing(shapes: ShapeView[], sizing: types.StackSizing, direction: PaddingDir) {
        const op = this.__repo.start("modifyAutoLayoutGapSizing");
        try {
            for (let i = 0, len = shapes.length; i < len; i++) {
                const view = shapes[i];
                const shape = shape4Autolayout(op, view, this.view);
                ...
                const h = view.frame.height;
                expandTo(op, this.__document, this.page, adapt2Shape(view), w, h);
                op.shapeModifyAutoLayoutGapSizing(this.page, shape, sizing, direction);
                op.shapeModifyAutoLayoutSizing(this.page, shape, types.StackSizing.Fixed, direction);
            }
            this.__repo.commit();
        } catch (e) {
            console.error(e);
    ...
    modifyAutoLayoutZIndex(shapes: ShapeView[], stack: boolean) {
        const op = this.__repo.start("modifyAutoLayoutZIndex");
        try {
            for (let i = 0, len = shapes.length; i < len; i++) {
                const shape = shape4Autolayout(op, shapes[i], this.view);
                op.shapeModifyAutoLayoutStackZIndex(this.page, shape, stack);
            }
            this.__repo.commit();
        } catch (e) {
            console.error(e);
    ...
    modifyAutoLayoutStroke(shapes: ShapeView[], included: boolean) {
        const op = this.__repo.start("modifyAutoLayoutStroke");
        try {
            for (let i = 0, len = shapes.length; i < len; i++) {
                const shape = shape4Autolayout(op, shapes[i], this.view);
                op.shapeModifyAutoLayoutStroke(this.page, shape, included);
            }
            this.__repo.commit();
        } catch (e) {
            console.error(e);
    ...
    adapt(views: ShapeView[]) {
        try {
            ...
            for (const view of views) {
                ...
                adapt_for_artboard(op, this.page, view);
            }
            this.__repo.commit();
        } catch (error) {
            this.__repo.rollback();
...
function getFillMaskVariable(op: Operator, page: PageView, view: ShapeView, value: any) {
    return _ov(VariableType.FillsMask, OverrideType.FillsMask, () => value, view, page, op);
}

function getBorderFillMaskVariable(op: Operator, page: PageView, view: ShapeView, value: any) {
    return _ov(VariableType.BorderFillsMask, OverrideType.BorderFillsMask, () => value, view, page, op);
}

function getBorderMaskVariable(op: Operator, page: PageView, view: ShapeView, value: any) {
    return _ov(VariableType.BordersMask, OverrideType.BordersMask, () => value, view, page, op);
}

function getRadiusMaskVariable(op: Operator, page: PageView, view: ShapeView, value: any) {
    return _ov(VariableType.RadiusMask, OverrideType.RadiusMask, () => value, view, page, op);
}

function getShadowMaskVariable(op: Operator, page: PageView, view: ShapeView, value: any) {
    return _ov(VariableType.ShadowsMask, OverrideType.ShadowsMask, () => value, view, page, op);
}

function getBlurMaskVariable(op: Operator, page: PageView, view: ShapeView, value: any) {
