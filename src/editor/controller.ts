...
import {ContactRoleType, CurveMode, ShapeType, SymbolShape} from "../data/typesdefine";
...
import {get_state_name, is_state, unable_to_migrate} from "./utils";
    ...
    public asyncTransfer(shapes: Shape[], page: Page): AsyncTransfer {
        ...
        const migrate = (targetParent: GroupShape, sortedShapes: Shape[]) => {
            ...
            for (let i = 0, len = sortedShapes.length; i < len; i++) {
                ...
                if (unable_to_migrate(targetParent, shape)) continue;
