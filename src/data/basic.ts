...
export class MultiResourceMgr<T> extends WatchableObject {
    private __resource = new Map<string, Array<T>>()
    private __guard?: (data: T) => T;
    private __updater?: (data: T) => void;
    private __loading: Map<string, {
        id: string,
        resolves: ((v: Array<T> | undefined) => void)[],
        rejects: ((e?: any) => void)[]
    }> = new Map();
    private __crdtpath: string[];
    ...
    async get(id: string): Promise<Array<T> | undefined> {
        let r = this.__resource.get(id)
    ...
    getSync(id: string): Array<T> | undefined {
        return this.__resource.get(id)
    }
    add(id: string, r: T) {
        r = this.__guard && this.__guard(r) || r
