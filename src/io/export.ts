...
import { Page, SymbolShape, SymbolRefShape, ImageShape, ArtboardRef } from "../data/classes";
import * as types from "../data/typesdefine"
import { exportPage, exportSymbolShape, exportArtboard, IExportContext, exportDocumentMeta } from "./baseexport";

export interface ExFromJson {
    document_meta: types.DocumentMeta,
    pages: types.Page[],
    page_refartboards: string[][],
    page_refsyms: string[][], // 不包含artboard引用的symbol，最后用artboards的合并就行
...
export async function exportExForm(document: Document): Promise<ExFromJson> {
    ...
    for (let i = 0, len = document.pagesList.length; i < len; i++) {
        ...
        const page = exportPage(ctx, pagedata);
        pages.push(page)
    ...
    for (let i = 0, len = page_refartboards.length; i < len; i++) {
        ...
        for (let j = 0, jlen = arr.length; j < jlen; j++) {
            ...
            const ab = exportArtboard(ctx, abdata);
            artboards.push(ab)
    ...
    const f = async (refsyms: string[][]) => {
        for (let i = 0, len = refsyms.length; i < len; i++) {
            ...
            for (let j = 0, jlen = arr.length; j < jlen; j++) {
                ...
                const sym = exportSymbolShape(ctx, symdata);
                symbols.push(sym);
            }
        }
    }
    ...
    ctx.allmedias.forEach(async (media_id) => {
    ...
    // document meta
    const document_meta = exportDocumentMeta(ctx, document);
