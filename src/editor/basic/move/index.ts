...
export class ShapePorter {
    ...
    constructor(op: Operator, page: Page) {
        this.op = op;
        this.page = page;
    ...
    private solidifyVar(view: ShapeView, target: GroupShapeView) {
        ...
        if (name === shape.name) op.shapeModifyName(page, shape, name);
        const fills = view.getFills().map(i => importFill(exportFill(i)));
        {
            if (shape.style.fills.length) op.deleteFills(shape.style.fills, 0, shape.style.fills.length);
            op.addFills(shape.style.fills, fills); // 填入新的值
        }
        let borders = view.getBorder();
        {
            if(borders) importBorder(exportBorder(borders))
            if (shape.style.borders?.strokePaints.length) op.deleteStrokePaints(page, shape, 0, shape.style.borders.strokePaints.length);
            op.addStrokePaints(page, shape, borders.strokePaints);
        }
        const shadows = view.getShadows().map(i => importShadow(exportShadow(i)));
        {
            if (shape.style.shadows.length) op.deleteShadows(shape.style.shadows, 0, shape.style.shadows.length);
            op.addShadows(shape.style.shadows, shadows);
        }
        const visible = view.isVisible;
        if (visible !== shape.isVisible) {
            op.shapeModifyVisible(page, shape, visible);
        }
        const lock = view.isLocked;
        if (lock !== shape.isLocked) op.shapeModifyLock(page, shape, lock);
        const contextSettings = view.contextSettings;
        {
            if (contextSettings?.opacity !== shape.style.contextSettings?.opacity) {
                op.shapeModifyContextSettingsOpacity(page, shape, contextSettings?.opacity ?? 1);
            }
            if (contextSettings?.blenMode !== shape.style.contextSettings?.blenMode) {
                op.shapeModifyContextSettingsBlendMode(page, shape, contextSettings?.blenMode ?? BlendMode.Normal);
            }
        }
        const startMarkerType = view.startMarkerType;
        if (startMarkerType !== shape.style.startMarkerType) op.shapeModifyStartMarkerType(page, shape, startMarkerType ?? MarkerType.Line);
        const endMarkerType = view.endMarkerType;
        if (endMarkerType !== shape.style.endMarkerType) op.shapeModifyEndMarkerType(page, shape, endMarkerType ?? MarkerType.Line);
        const exportOptions = view.exportOptions ? importExportOptions(exportExportOptions(view.exportOptions)) : undefined;
        {
            if (shape.exportOptions?.exportFormats.length) op.deleteExportFormats(page, shape, 0, shape.exportOptions.exportFormats.length);
            if (exportOptions?.exportFormats.length) op.addExportFormats(page, shape, exportOptions.exportFormats);
        }
        const cornerRadius = view.cornerRadius;
        {
            if (cornerRadius) op.shapeModifyRadius2(page, (shape as Artboard), cornerRadius.lt, cornerRadius.rt, cornerRadius.rb, cornerRadius.lb);
        }
        const blur = view.blur ? importBlur(exportBlur(view.blur)) : undefined;
        {
            if (blur) op.addBlur(shape.style, blur);
            else if (shape.style.blur) op.deleteBlur(shape.style);
        }
        const protoInteractions = view.prototypeInteractions
            ...
            : undefined;
        {
            if (shape.prototypeInteractions?.length) shape.prototypeInteractions.forEach(i => op.deleteShapePrototypeInteractions(page, shape, i.id));
            if (protoInteractions?.length) protoInteractions.forEach(i => op.insertShapeprototypeInteractions(page, shape, i));
        }

        if (view instanceof TextShapeView) {
            const text = view.getText();
            const textShape = shape as TextShape;
            op.deleteText2(page, textShape, 0, textShape.text.length);
            op.insertComplexText(page, view, 0, text);
        }
        ...
        if (view instanceof ArtboardView) {
            ...
            autoLayout && op.shapeAutoLayout(page, shape as Artboard, autoLayout);
        }
    }
    ...
    transform(view: ShapeView, target: GroupShapeView, shape?: Shape) {
        ...
        this.op.shapeModifyTransform(this.page, shape ?? adapt2Shape(view), (transform));
    }
    ...
    move(view: ShapeView, origin: GroupShapeView, target: GroupShapeView, toIndex: number) {
        ...
        this.op.shapeMove(this.page, originData, index, targetData, toIndex);
