...
const shadowOri: { [key: string]: (h: Function, style: Style, frame: ShapeFrame, id: string, i: number, path: string) => any } = {};
...
function shadowType(h: Function, shape: Shape, i: number, id: string, pathstr: string, comsMap?: Map<ShapeType, any>): any {
  ...
  if (shape.type === ShapeType.Image) {
    ...
    border = renderB(h, shape.style, pathstr!, shape);
    ...
    border = border.concat(renderGroupChilds(h, shape as GroupShape, comsMap));
    const p = h('g', g_props, border);
    return { filter, p }
  }
...
export function render(h: Function, id: string, path: string, shape: Shape, comsMap?: Map<ShapeType, any>) {
  ...
  for (let i = 0; i < shadows.length; i++) {
    ...
    if (position === ShadowPosition.Outer) {
      if (shape.type === ShapeType.Rectangle || shape.type === ShapeType.Artboard || shape.type === ShapeType.Oval) {
        ...
        const { filter, p } = shadowType(h, shape, i, id, path, comsMap);
        filters.push(filter);
        paths.push(p);
      }
